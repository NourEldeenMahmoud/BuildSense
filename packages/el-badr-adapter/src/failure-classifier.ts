...
export function classifyElBadrHttpFailure(input: HttpFailureInput): ScrapeFailureKind {
  const { httpStatus, contentType, message, redirectUrl } = input;

  if (redirectUrl != null) {
    try {
      const redirectHost = new URL(redirectUrl).hostname;
      if (redirectHost !== EL_BADR_HOST && !redirectHost.endsWith('.elbadrgroupeg.store')) {
