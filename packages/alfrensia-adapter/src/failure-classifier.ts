...
const ALFRENSIA_HOST = 'alfrensia.com';

export function classifyAlfrensiaHttpFailure(input: HttpFailureInput): ScrapeFailureKind {
  const { httpStatus, contentType, message, redirectUrl } = input;

  if (redirectUrl != null) {
    try {
      const redirectHost = new URL(redirectUrl).hostname;
      if (redirectHost !== ALFRENSIA_HOST && !redirectHost.endsWith('.alfrensia.com')) {
