...
import type { AdminMatchReviewListItem, AdminPagination, AdminMatchReviewStatus } from '@buildsense/contracts';

type LoadState = 'loading' | 'loaded' | 'error';

@Component({
  selector: 'app-admin-match-reviews',
  standalone: true,
            ...
            <tbody>
              @for (review of items(); track review.id) {
                <tr class="data-tr">
                  ...
                  <td class="data-td data-td--url">
                    <a class="run-link" [routerLink]="['/admin/match-reviews', review.id]">
...
export class AdminMatchReviewsPage implements OnInit {
  private readonly api = inject(AdminApiService);

  readonly state = signal<LoadState>('loading');
  readonly items = signal<AdminMatchReviewListItem[]>([]);
  ...
  load(): void {
    ...
    this.api.getMatchReviews(query).subscribe({
      next: (data) => {
  ...
  formatStatus(status: AdminMatchReviewStatus): string {
    const map: Record<AdminMatchReviewStatus, string> = {
      PENDING: 'PENDING',
      LINKED: 'LINKED',
      IGNORED: 'IGNORED',
