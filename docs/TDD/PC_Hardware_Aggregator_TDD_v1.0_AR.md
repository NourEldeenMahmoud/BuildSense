...
| نطاق التنفيذ | Baseline بدأ بمتجر Sigma؛ التنفيذ الحالي يدعم بيانات أربعة متاجر مستقلة |
...
> **حالة التنفيذ الحالية:** توسعت بنية الـStore Adapters إلى Sigma Computer وEl Badr Group وEl
> Nour Tech وAlfrensia Computer. الـWorker وحده يملك الإدخال، وكل متجر يحتفظ بمنطق الاستخراج
> الخاص به. التوافق تجريبي ومقيد بتغطية وprecision وverified-sample gates؛ النتيجة تبقى
> `UNKNOWN` عندما لا تكفي الأدلة. قرارات ADR المقبولة وملفات `package.json` الحالية تتقدم على
> أمثلة التنفيذ التاريخية في هذه الوثيقة.
...
3. **Worker CLI**: اكتشاف أو استيراد صفحات المتاجر المدعومة، حفظ Raw snapshots، Normalization، Product identity resolution، ونشر البيانات.
...
```text
Store Website / Approved Capture
     |
     v
Worker
...
- **Offer model** منفصل لدعم عروض متعددة من متاجر مستقلة.
...
| ----------------- | ------------------- | ------------------------------------------------------------------- |
| Frontend          | Angular 19          | Standalone components، Signals، Reactive Forms، Router، HttpClient. |
| Runtime           | Node.js 24 LTS      | نفس Runtime للـAPI والـWorker، ومتوافق مع Angular 19.               |
| Language          | TypeScript 5.8      | Strict mode وتوحيد الأنواع بين التطبيقات.                           |
| API               | Express 4.21        | REST API بسيطة ومألوفة، مع Middleware صريح.                         |
| Database          | MongoDB + Atlas     | Atlas هو الإعداد الافتراضي للتطوير؛ MongoDB محلي اختياري.           |
| DB access         | Mongoose 8          | Models وRepository layer صريحة مع validation.                       |
...
| Monorepo          | npm workspaces + Nx | Targets موحدة وحدود dependencies صريحة.                              |
| Local environment | Host processes      | Web وAPI محليان؛ Worker أوامر one-shot؛ MongoDB Atlas افتراضي.       |

### لماذا Mongoose؟

- Models مركزية مع validation وindexes واضحة داخل `packages/database`.
- Repository layer تمنع تسرب تفاصيل Mongoose إلى الـWeb أو الـDomain.
- API والـWorker يشتركان في طبقة البيانات دون أن يملك أحدهما منطق الآخر.
...
- نسخة MongoDB المستضافة أو المحلية يجب أن تكون متوافقة مع Models وIndexes الحالية.
...
| Store adapters | Store URLs، selectors، discovery/import، parser، stock/price interpretation. |
| `compatibility-facts` | استخراج facts موحدة ومقيسة من مواصفات المنتجات. |
...
2. Background admin jobs claimed from MongoDB.
3. One-off maintenance commands مثل extract أو import أو publish.
...
- `development`: Local web/api + one-shot Worker commands + MongoDB Atlas أو MongoDB محلي.
- `test`: MongoDB Memory Server مع عزل حالة الاختبارات.
...
- زر لكل منتج يفتح الـOffer المختار في متجره الأصلي.
- زر `Open available items` اختياري لكن لا يعتمد عليه بسبب popup blocking.
- Checklist توضح ما تم فتحه يدويًا في الواجهة فقط.
- لا يتم إرسال Cart data إلى أي متجر بدون Integration رسمي.

## 18.3 Multi-store

- كل item يختار Offer متاحًا.
- الخطة تجمع العناصر حسب المتجر وتعرض رابط كل Offer.
- إتمام الشراء منفصل داخل كل متجر؛ لا يوجد split payment أو checkout داخل BuildSense.
- استراتيجيات اختيار العروض المتقدمة تظل مستقبلية.
...
- all raw store specifications.
- data quality note إن كانت critical fields ناقصة.
- رابط المتجر الأصلي لكل Offer.
...
تعرف أنواع الـreference data داخل `packages/domain/src/reference-data.ts`، وتخزن النسخة المنشورة
في MongoDB عبر `ReferenceDataset`. لا توجد صفحة Admin مستقلة باسم Reference Data؛ تستخدم قواعد
التوافق النسخة المنشورة مباشرة.
...
اختبارات الـTransactions تستخدم MongoDB Memory Server في وضع replica set عند الحاجة.
...
Host:
  ...
  API process
  Worker CLI commands
MongoDB Atlas (default) or a local MongoDB instance
```
...
- عدد محدود من المتاجر المستقلة والعروض المتعددة لكل منتج.
...
- MongoDB Atlas configuration or a local MongoDB instance.
- Pino logging.
...
5. `M0-005` Add MongoDB client and Atlas/local configuration.
...
القرارات المقبولة والمرقمة محفوظة في `docs/ADR/`. أي قرار جديد يضاف هناك مع Context, Decision,
Alternatives, and Consequences، ولا يعاد استخدام رقم ADR قائم.
...
The canonical root scripts are maintained in `package.json`. The supported entry points are the Nx
`dev`, `build`, `test`, `lint`, and `typecheck` targets; `npm run worker -- <command>` for Worker CLI
operations; the `bootstrap:*` pipeline; Sigma fixture generators; and `capture:browser`.
